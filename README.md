...
## 💡 매일 사용하는 올바른 실행 순서 (매우 중요)
카메라 장치(디바이스)를 점유하는 순서가 꼬이면 화면이 나오지 않을 수 있습니다. 반드시 아래 순서대로 켜주세요.

1.  **OBS Virtual Camera 켜기**: OBS Studio에서 가상 카메라 시작 버튼을 누르거나 장치가 활성화되어 있는지 확인합니다.
2.  **출석 서버(본 프로그램) 켜기**: `uv run uvicorn app.main:app --host 0.0.0.0 --port 5000 --reload` 명령어로 파이썬 서버를 가장 먼저 실행합니다. (이때 서버가 실제 물리 웹캠을 안전하게 선점합니다.)
3.  **외부 접속용 터미널 켜기 (선택)**: 스마트폰 접속이 필요하다면 새 창에서 `ngrok http 5000`을 실행합니다.
4.  **출석 체크용 프로그램(타겟) 켜기**: Zoom, Teams, 사내 웹 출결 프로그램, 또는 테스트용 `dummy_QR_Camera.py`를 **마지막에 실행**합니다. 카메라 설정에서 입력 장치를 **"OBS Virtual Camera"** 로 선택합니다.

이렇게 하면 '물리 웹캠' -> '본 서버' -> '가상 카메라' -> '출석 프로그램' 순서로 물려 정상적으로 송출됩니다.
